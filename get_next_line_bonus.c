...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if ((remaining[fd] && !ft_strchr(remaining[fd], '\n')) || !remaining[fd])
		remaining[fd] = ft_get_remaining(fd, remaining[fd]);
	if (!remaining[fd])
		return (NULL);
	next_line = ft_get_the_line(remaining[fd]);
	if (!next_line)
	{
		free(remaining[fd]);
		return (NULL);
	}
	remaining[fd] = ft_update_remaining(remaining[fd]);
	return (next_line);
}
